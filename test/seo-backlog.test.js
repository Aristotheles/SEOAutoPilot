...
test('keeps the agreed LingoDecoder evergreen recommendations in a persistent queue', () => {
  const first = mergeEditorialBacklog('lingodecoder', [], []);
  ...
  const refreshed = mergeEditorialBacklog('lingodecoder', [], [preserved]);
...
test('does not duplicate an active Search Console target', () => {
  ...
  const merged = mergeEditorialBacklog('lingodecoder', active, []);
