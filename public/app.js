...
function workflowActionPanel(workflow, targetUrl) {
  ...
  if (workflow.status === 'APPROVED') return `<div class="apply-progress monitoring"><i>↗</i><div><strong>Değişiklikler hazırlanmaya hazır</strong><p>Tek tek işlem yapmak yerine toplu yayın ekranını kullanabilirsin.</p></div></div><div class="detail-actions"><button class="modal-submit" data-open-bulk-publish>Toplu yayın ekranına git <span>→</span></button></div>`;
  ...
  if (workflow.status === 'PREVIEW_READY') return `<div class="apply-progress done"><i>✓</i><div><strong>Değişiklikler yayına hazır</strong><p>${escapeHtml(formatDateTime(workflow.execution?.previewAt))} · ${workflow.execution?.appliedChangeIds?.length || 0} değişiklik hazırlandı.</p></div></div><div class="detail-actions"><button class="modal-submit" data-open-bulk-publish>Toplu yayın ekranına git <span>→</span></button></div>`;
