...
function renderDataState() {
  $('#connectionManagementProject').textContent = state.project ? `${AppI18n.t('Seçili proje:')} ${state.project.name} — ${state.project.siteUrl}` : '';
  ...
  $('#deploymentSourceText').textContent = ready ?
    `${deployment.connection.framework} · ${deployment.connection.provider} · ${deployment.connection.branch} ${AppI18n.t('dalı')} · ${deployment.connection.sourceDirectory || 'web'} → ${deployment.connection.outputDirectory || 'build/web'} · ${deployment.untrackedFiles || 0} ${AppI18n.t('izlenmeyen dosya')} · ${AppI18n.t('Firebase hesabı')}: ${deployment.firebaseAccess?.account || AppI18n.t('doğrulanmadı')} · ${AppI18n.t('Erişim kontrolü')}: ${formatDateTime(deployment.firebaseAccess?.checkedAt)}${deployment.publicationWarning ? ` · ${deployment.publicationWarning}` : ''}` :
    AppI18n.t('Onaylanan değişikliklerin gerçekten uygulanması için yerel Git ve yayınlama bağlantısı gerekir.');
}
...
function monitoringReportHtml(workflow, targetUrl) {
  const result=workflow.result;if(!result?.baseline||!result?.current)return `<div class="apply-progress done"><i>✓</i><div><strong>${AppI18n.t('28 günlük izleme tamamlandı')}</strong><p>${escapeHtml(result?.recommendation||AppI18n.t('Güncel Search Console verisini yenileyip sonucu tekrar incele.'))}</p></div></div>`;
  const label=AppI18n.t({improved:'Olumlu sonuç',declined:'Gerileme var',stable:'Belirgin değişim yok',insufficient_data:'Veri yetersiz'}[result.verdict]||'Kontrol gerekli');
  const delta=(value,suffix='')=>`${value>0?'+':''}${number(value,suffix?1:0)}${suffix}`;
  return `<div class="apply-progress done"><i>✓</i><div><strong>${AppI18n.t('28 günlük izleme tamamlandı')} · ${label}</strong><p>${escapeHtml(result.recommendation)}</p></div></div><div class="evidence-grid monitoring-comparison"><article><small>Tıklama</small><strong>${number(result.baseline.clicks)} → ${number(result.current.clicks)}</strong><small>${delta(result.changes.clicks*100,'%')}</small></article><article><small>Gösterim</small><strong>${number(result.baseline.impressions)} → ${number(result.current.impressions)}</strong><small>${delta(result.changes.impressions*100,'%')}</small></article><article><small>CTR</small><strong>${percent(result.baseline.ctr)} → ${percent(result.current.ctr)}</strong><small>${delta(result.changes.ctr*100,' puan')}</small></article><article><small>Ort. konum</small><strong>${number(result.baseline.position,1)} → ${number(result.current.position,1)}</strong><small>${AppI18n.t(result.changes.position>0?'İyileşme':'Değişim')}: ${delta(result.changes.position)}</small></article></div><div class="detail-actions"><a class="outline-button view-page-link" href="${escapeHtml(workflow.execution?.url||targetUrl)}" target="_blank" rel="noopener">${AppI18n.t('İlgili sayfayı gör ↗')}</a></div>`;
}
function workflowActionPanel(workflow, targetUrl) {
  ...
  if (workflow.status === 'PREVIEW_READY') return `<div class="apply-progress done"><i>✓</i><div><strong>Değişiklikler yayına hazır</strong><p>${escapeHtml(formatDateTime(workflow.execution?.previewAt))} · ${workflow.execution?.appliedChangeIds?.length || 0} ${AppI18n.t('değişiklik hazırlandı. Canlı site henüz değişmedi.')}</p></div></div><div class="detail-actions"><button class="outline-button" data-open-bulk-publish>Toplu yayın ekranına git</button><button class="modal-submit" data-workflow-publish="${escapeHtml(workflow.id)}">Yalnız bu sayfayı canlıya yayınla <span>→</span></button></div>`;
  ...
  if (['PUBLISHED', 'APPLIED'].includes(workflow.status)) return `<div class="apply-progress done"><i>✓</i><div><strong>Sayfa canlıya yayınlandı</strong><p>${escapeHtml(formatDateTime(workflow.execution?.appliedAt))} ${AppI18n.t('tarihinde yayın adresi doğrulandı.')}</p></div></div><div class="detail-actions"><a class="outline-button view-page-link" href="${escapeHtml(workflow.execution?.url || targetUrl)}" target="_blank" rel="noopener">Yeni sayfayı gör ↗</a><button class="modal-submit" data-workflow-action="start_monitoring" data-workflow-id="${escapeHtml(workflow.id)}">İzlemeyi başlat <span>→</span></button></div>`;
  if (workflow.status === 'MONITORING') { const interim=workflow.monitoringInterim; const base=workflow.monitoringBaseline; return `<div class="apply-progress monitoring"><i>◷</i><div><strong>14/28 günlük ölçüm sürüyor</strong><p>${AppI18n.t('Yayın ve baz çizgisi:')} ${escapeHtml(formatDateTime(workflow.monitoringStartedAt))}${base?.period ? ` · ${AppI18n.t('Önceki dönem')} ${escapeHtml(base.period.start)}–${escapeHtml(base.period.end)}` : ''}. ${AppI18n.t('Sonuç yalnız yayın sonrası veri dönemi tamamlanınca hazırlanır.')}</p></div></div>${interim?`<div class="approval-box"><strong>${AppI18n.t('14 günlük ara kontrol:')} ${escapeHtml(interim.recommendation)}</strong><br>${AppI18n.t('Bu ara bilgidir; 28 günlük nihai karar değildir.')}</div>`:''}${workflow.execution?.url ? `<div class="detail-actions"><a class="outline-button view-page-link" href="${escapeHtml(workflow.execution.url)}" target="_blank" rel="noopener">Yayınlanan sayfayı gör ↗</a></div>` : ''}`; }
...
function renderProjects() {
  ...
  $$('.nav-item').forEach((button) => {
    ...
    button.title = needsProject ? `${AppI18n.t(viewLabels[button.dataset.view])} ${AppI18n.t('için önce bir proje ekle')}` : '';
  });
  $('#currentProjectName').textContent = state.project?.name || AppI18n.t('Proje ekle');
  $('#breadcrumbProject').textContent = state.project?.name || AppI18n.t('Proje yok');
...
function openProjectCenter(requestedView = null) {
  ...
  $('#projectModalHint').textContent = requestedView ?
    `${AppI18n.t(viewLabels[requestedView])} ${AppI18n.t('bölümünü kullanmak için önce bir proje ekle. Projeyi oluşturduğunda bu bölüm otomatik açılacak.')}` :
    AppI18n.t('Her site kendi verisi, Search Console bağlantısı ve öneri kuyruğuyla çalışır.');
...
async function removeConnection(kind, projectId = state.project?.id) {
  ...
  const messages = {
    deployment: `${name}\n\n${AppI18n.t('Site/Git/Firebase bağlantısı kaldırılacak. Eski önizlemenin yayın yetkisi iptal edilir. Canlı site, kaynak klasörü ve GitHub deposu silinmez. Diğer projeler etkilenmez. Devam edilsin mi?')}`,
    google: `${name}\n\n${AppI18n.t('Bu projenin Google tokenları silinecek ve otomatik senkronizasyonu duracak. Raporlar ve diğer projeler korunur. Google hesabındaki izin iptal edilmez. Devam edilsin mi?')}`,
    'google-config': `${AppI18n.t('DİKKAT: TÜM')} ${state.projects.length} ${AppI18n.t('PROJEYİ ETKİLER.')}\n\n${AppI18n.t("Ortak Google OAuth istemci kimliği, gizli anahtar ve tüm projelerin Google tokenları bu uygulamadan silinecek. Ortam değişkenleriyle yapılandırılmış erişim de bu uygulamada devre dışı bırakılır. Yeniden kurulum gerekir. Google Cloud API/anahtarı silinmez; Google hesabındaki izin iptal edilmez. Siteler ve raporlar korunur. Devam edilsin mi?")}`,
  };
  if (kind === 'project') {
    const answer = window.prompt(`${name}\n\n${AppI18n.t('Bu projenin SEOAutoPilot kaydı, raporları, görevleri ve tokenları kalıcı olarak silinir. Canlı site, kaynak kod, GitHub, CSV dosyaları ve diğer projeler korunur. Google hesabındaki izin iptal edilmez.')}\n\n${AppI18n.t('Onaylamak için proje adını aynen yaz:')} ${project.name}`);
