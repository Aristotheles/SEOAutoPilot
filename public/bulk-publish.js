...
$('#reviewBulkPublish').addEventListener('click',async()=>{
  ...
  try{
    ...
    if(!r.items.length){$('#bulkPublishContent').innerHTML=`<p><strong>Şu anda tek düğmeyle yayınlanabilecek hazır değişiklik yok.</strong></p><p>${r.skipped.length?'Aşağıdaki öneriyi inceleyip karar vermen gerekiyor.':'Senden beklenen bir işlem yok.'}</p>${skippedDetails}`;return;}
    const lines=r.items.map((i,index)=>`${index+1}. ${i.title}\n   ${i.changes.map(c=>`${c.area}: ${String(c.proposed).slice(0,90)}`).join('\n   ')}`).join('\n\n');
    const accepted=window.confirm(`${r.items.length} ${AppI18n.t('sayfa canlıya yayınlanacak:')}\n\n${lines}\n\n${r.skipped.length} ${AppI18n.t('gerçek öneri ayrıca karar bekliyor. Yalnız listelenen başlık, meta ve H1 değişiklikleri uygulanır. İlk hatada kalan kuyruk durur; yayınlanan sayfalar otomatik geri alınmaz.')}\n\n${AppI18n.t('Hepsini şimdi canlıya yayınla?')}`);
